...
my $in1_file = $outpath.'/'.$sub.'/retro_v'.$ver.'_1/ALU/'.$sub.'.pred.R1.txt';
my $in2_file = $outpath.'/'.$sub.'/retro_v'.$ver.'_1/ALU/'.$sub.'.pred.R2.txt';
my $ou1_file = $outpath.'/'.$sub.'/retro_v'.$ver.'_0/ALU/RS'.$cutoff.'.0strand.bed';
my $ou2_file = $outpath.'/'.$sub.'/retro_v'.$ver.'_1/ALU/RS'.$cutoff.'.1strand.bed';
...
system("$masterpath/ALU/06_Predict/03_filter_control.sh $outpath $ver $sub $cont $cutoff 1 $masterpath $hg");
system("$masterpath/ALU/06_Predict/03_filter_control.sh $outpath $ver $sub $cont $cutoff 0 $masterpath $hg");

my $in_file1 = $outpath.'/'.$sub.'/retro_v'.$ver.'_0/ALU/noCONT.cutoff'.$cutoff.'.calls';
my $in_file2 = $outpath.'/'.$sub.'/retro_v'.$ver.'_1/ALU/noCONT.cutoff'.$cutoff.'.calls';
my $out_file = $outpath.'/'.$sub.'/retro_v'.$ver.'_1/ALU/cutoff'.$cutoff.'.candidate.calls';
...
my ($pe_file, $sr_file);
    ...
    for ($strand=0; $strand <2; $strand ++)
       {
        $pe_file = $outpath.'/'.$sub.'/retro_v'.$ver.'_'.$strand.'/'.$sub.'.pe.ALU.matrix';
        $sr_file = $outpath.'/'.$sub.'/retro_v'.$ver.'_'.$strand.'/'.$sub.'.sr.ALU.matrix';
