...
$pe_files[0] = $ARGV[9].'/04SE_PE.alignment';
$sr_files[0] = $ARGV[9].'/05SE_SR.alignment';
$pe_files[1] = $ARGV[9].'/04SF_PE.alignment';
$sr_files[1] = $ARGV[9].'/05SF_SR.alignment';
$refs[0]    = $ARGV[0].'/refTE/sequence/SVA_E.fa';
$refs[1]    = $ARGV[0].'/refTE/sequence/SVA_F.fa';
my $chr     = $ARGV[2];
my $te      = $ARGV[3];
my $hg      = $ARGV[5];
my $strand  = $ARGV[6];
            ...
            if ( $strand && ($cigar =~ /^(\d+)S/) )
               {
                ...
                $im -> line($x3,$y3,$x4,$y4,$black);
                $x3 = $min - $anchor_len*$scale + $data[6] * $scale;
                $x4 = $min - $anchor_len*$scale + ($data[7]-1) * $scale;
